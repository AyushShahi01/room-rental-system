...
3. **Configure:**
   ...
   - **Environment:** Python
   - **Build Command:** `cd backend/project && pip install -r requirements.txt && python manage.py migrate --noinput && python manage.py collectstatic --noinput`
...
The build command runs `python manage.py migrate --noinput` on every deploy, so
new database migrations are applied before the service starts.
